...
ull modPow(ull b,ull exponent)
{
    ...
    ull temp = modPow(b, exponent/2);

    if (exponent %2 ==0)
        return (temp%mod * temp%mod) % mod;
    else
        return ((( temp%mod * temp%mod)%mod) * b%mod) % mod;
}
