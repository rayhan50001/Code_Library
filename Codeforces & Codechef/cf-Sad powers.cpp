...
ll _sqrt(ll n){ll e=sqrt(n*1.0);ll l=max(0LL,e-2),r=min(n,e+2);ll ans=0;while(l<=r){ll m=Mid(l,r);if(m*m<=n)ans=m,l=m+1;else r=m-1;}return ans;}
...
void calc()
{
    ...
    {
        ll sqq=sqrt(i);
        if(sqq*sqq==i)continue;
        ll lg=log(1000000000010)/log(i);
        for(ll j=3; j<=lg; j+=2)
        {
            ll val=power(i,j);
            if(val<=1000000000010 && val>0)
            {
                st.insert(val);
            }
            else break;
...
//    //your code goes here
    ...
    while(tt--)
    {
        ll l,r;
        sfll2(l,r);
        ll low=lower_bound(sq.begin(),sq.end(),l)-sq.begin();
        ll high=upper_bound(sq.begin(),sq.end(),r)-sq.begin();
        ll perfect_l=_sqrt(l-1);
        ll perfect_r=_sqrt(r);
        cout<<perfect_l<<" "<<perfect_r<<endl;
        ll ans=(high+perfect_r)-(low+perfect_l);
        pfll(ans);
        nl;
    }
//    //end here
