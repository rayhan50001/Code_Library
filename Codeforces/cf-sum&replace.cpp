...
void build(ll idx,ll l,ll r)
{
    ...
    one_two[idx]=one_two[lft]+one_two[rgt];
}
void update(ll idx,ll l,ll r,ll s,ll e)
{
    if(r < s || l > e)
    {
        return;
    }
    if(one_two[idx]==(r-l+1))return;//check for this segment is only 1/2
    if(l==r)
    {
        tree[idx]=divi[tree[idx]];
        if(tree[idx]<=2)one_two[idx]=1;
        return;
    }
    ll mid=(l+r)>>1;
    ll lft=2*idx;
    ll rgt=lft+1;
    update(lft,l,mid,s,e);
    update(rgt,mid+1,r,s,e);
    tree[idx]=tree[lft]+tree[rgt];
    one_two[idx]=one_two[lft]+one_two[rgt];
}
