...
trienode Tree[100001];
void newnode()
{
    Tree[start].c = 0;
    memset(Tree[start].child, 0, sizeof Tree[start].child);
    start++;
    Tree[start].endofword=false;
}
void init()
{
    start=1;
...
void Insert(string s)
{
    ...
    for(int i=0; i<s.size(); i++)
    {
        ...
        if(!Tree[root].child[index])
        {
            Tree[root].child[index]=start;
            newnode();
        }
        root=Tree[root].child[index];
    }
    Tree[root].endofword=true;
    Tree[root].c++;
}
ll Search(string s)
{
