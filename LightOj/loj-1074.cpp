...
void check(int u,int n)
{
    if(cycle[u])return;
    cycle[u]=1;
    for(int i=1; i<=n; i++)if(graph[u][i]!=INT_MAX)check(i,n);
}
int main()
{
//    clock_t begin = clock();
    ...
    while(tt--)
    {
        ...
        for(int i=0; i<ed; i++)
        {
            ...
            edge[i].src=u;
            edge[i].dest=v;
            edge[i].weit=(val[v]-val[u])*(val[v]-val[u])*(val[v]-val[u]);
            graph[u][v]=edge[i].weit;
        }
        ...
        for(int i=0; i<ed; i++)
        {
                ...
                int v=edge[i].dest;
                int w=edge[i].weit;
                if(dist[u]!=INT_MAX && dist[u]+w<dist[v] && !cycle[u])
                    check(u,n);
        }
        int q;
        sf(q);
        ...
        for(int i=0; i<q; i++)
        {
            ...
            sf(u);
            //cout<<dist[u]<<endl;
            if(dist[u]==INT_MAX || cycle[u] || dist[u]<3)printf("?");
            else pf(dist[u]);
            nl;
        }
