...
struct TRIE{
    ...
    void INSERT(int key)
    {
        ...
        {
            int bit=(key >> i) & 1;
            if(TREE[cur_node][bit]==-1)
            {
                TREE[cur_node][bit]=nodes;
                nodes++;
            }
    ...
    int maxXOR(int key)
    {
        ...
        {
            int bit=(key >> i) & 1;
            if(TREE[cur_node][bit^1]!=-1)
            {
                cur_node=TREE[cur_node][bit^1];
            }
            else if(TREE[cur_node][bit]!=-1)
            {
                cur_node=TREE[cur_node][bit];
            }
        }
        return value[cur_node]^key;
    }
    int minXOR(int key)
    {
        int cur_node=0;
...
//    //your code goes here
       ...
       while(tt--)
       {
          ...
          {
              //cout<<sum[i]<<" ";
              mx=max(mx,tree.maxXOR(sum[i]));
              mn=min(mn,tree.minXOR(sum[i]));
              tree.INSERT(sum[i]);
          }
          cs;
          pf(mx);
