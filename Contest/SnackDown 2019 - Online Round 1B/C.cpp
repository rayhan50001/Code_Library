...
int main()
{
    ...
    while(tt--)
    {
        ...
        for(ll i=0; i<v.size(); i++)
        {
            ...
            ll first=upper_bound(v.begin(),v.end(),T-1)-lower_bound(v.begin(),v.end(),1);
            first++;
            ll choose1=(first+m)*l-T;

            ll second=upper_bound(v.begin(),v.end(),ager_T-1)-lower_bound(v.begin(),v.end(),1);
            second++;
            ll choose2=(second+m)*l-ager_T;

            ll third=upper_bound(v.begin(),v.end(),porer_T-1)-lower_bound(v.begin(),v.end(),1);
            third++;
            ll choose3=(third+m)*l-porer_T;
            choose=min({choose,choose1,choose2,choose3});
        }

        ll last=upper_bound(v.begin(),v.end(),k-1)-lower_bound(v.begin(),v.end(),1);
        last++;
        ll ans=(last+m)*l-k;
        choose=min(choose,ans);
        pfll(choose);
        nl;
    }
}
