...
//    //your code goes here
       ...
       for(int i=1; i<=n; i++)
       {
           for(int j=i; j<=n; j++)
           {
               ...
               for(int k=1; k<=m; k++)
               {
                 int low=lower_bound(v[k].begin(),v[k].end(),l)-v[k].begin();
                 int high=upper_bound(v[k].begin(),v[k].end(),r)-v[k].begin();
                 summ+=(high-low);
                 if(coin[k]==(high-low))c++;
                 else
                 {
                     if((high-low)>coin[k])total+=(high-low)-coin[k];
                 }
               }
               if(c==m)
               {
