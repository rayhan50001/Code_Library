...
//    //your code goes here
     ...
     //WRITE;
      ...
      while(tt--)
      {
          ...
          //cout<<total<<endl;
          if(prev==1 && total>D)printf("IMPOSSIBLE");
          else if(total<=D)printf("0");
          else
          {
              ll cnt=0;
              ll ans=0;
              for(int i=s.size()-1; i>=0; i--)
              {
                  ...
                  else if(s[i]=='S' && cnt==0)
                  {
                      ...
                      ll left=total-D;
                      ll gun=prev/2;
                      ll lagbe=ceil((double)left/(double)gun);
                      total=total-(min(lagbe,cnt)*gun);
                      ans+=min(lagbe,cnt);
                      if(total<=D)
                      {
                          break;
                  ...
                  else
                  {
                      ...
                      total=total-(min(lagbe,cnt)*gun);
                      ans+=min(lagbe,cnt);
                      //cout<<total<<" "<<left<<" "<<lagbe<<" "<<total<<" "<<ans<<endl;
                      if(total<=D)
                      {
                          break;
                      }
