...
ll Distance(ll x1, ll y1, ll x2, ll y2) {			// Without Sqrt
	ll x = x1-x2;
	ll y = y1-y2;
	return x*x + y*y;
}

bool IntersectCircle(circle c1, circle c2) {
...
//    //your code goes here
    ...
    while(tt--)
    {
        ...
        for(int j=i+1; j<3; j++)
        {
            if(!Inside(v[i],v[j]) && IntersectCircle(v[i],v[j]))ans++;
        }
//        if(!Inside(v[0],v[1]) && !Inside(v[1],v[2]) && !Inside(v[0],v[2]))
//        {
//            if(IntersectCircle(v[0],v[1]) && IntersectCircle(v[1],v[2]) && IntersectCircle(v[0],v[2]))ans++;
//        }
        cs;
        pf(ans);
        nl;
