...
//    clock_t begin = clock();
    //    //your code goes here
    vector<ll>sq;
    for(ll i=0; i<40; i++)sq.pb(powl(2LL,i));
    int n;
    sf(n);
    for(int i=0; i<n; i++)
    {
    ...
    {
        int paisi=0;
        int pos=upper_bound(sq.begin(),sq.end(),arr[i])-sq.begin();
        for(int j=pos; j<sq.size(); j++)
        {
           ll rest=sq[j]-arr[i];
           if(arr[i]==rest && cnt[rest]==1)continue;
           if(binary_search(arr,arr+n,rest))
           {
            //cout<<arr[i]<<" "<<rest<<" "<<sq[j]<<endl;
            paisi=1;
            break;
           }
        }
