...
//    //your code goes here
    ...
    for(i=1; i<=n; i++)
    {
        ll nn=arr[i];
        int found=0;
        vector<pair<ll,ll> >fact=Factor::factorG(nn);
        //cout<<fact.size()<<endl;
        for(int j=0; j<fact.size(); j++)
        {
            int val=fact[j].first;
            //cout<<val<<endl;
            if(vis[val])
            {
                found=1;
    ...
    while(i<=n)
    {
        //cout<<prime[pos]<<" "<<vis[prime[pos]]<<endl;
        if(vis[prime[pos]]==1)pos++;
        else
        {
            arr[i]=prime[pos];
            i++;
