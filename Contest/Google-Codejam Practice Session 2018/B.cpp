...
pair<string,int> process()
{
            ...
            for(int i=1; i<=n; i++)
            {
                if(vis[i])
                {
                    double per=((double)vis[i]*100*1.00)/((double)total*1.00);
                    if(per>50.00)
                    {
                        if(id1){
                        vis[id1]+=1;
...
int ispossible(int id1,int id2)
{
     int chk=0;
     for(int i=1; i<=n; i++)
     {
...
//    //your code goes here
      ...
      while(tt--)
      {
         ...
         while(1)
         {
            pair<string,int> s1=process();
            pair<string,int> s2=process();
            int chk=ispossible(s1.second,s2.second);
//            for(int i=1; i<=n; i++)cout<<vis[i]<<" ";
//                cout<<endl;
//            cout<<s1.first<<" "<<s2.first<<" "<<chk<<endl;
            if(chk==100)
            {
                v.push_back(s1.first+s2.first);
            }
            else
            {
                if(vis[chk])
