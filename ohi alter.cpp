...
//}
#define MAX 100000
int Tree[4*MAX],arr[MAX],vis[MAX],ans[MAX];
...
int main()
{
    ...
    for1(i,1,n)
    {
        if(vis[arr[i]]!=-1)
        {
            Update(1,1,n,vis[arr[i]],0);
        }
        vis[arr[i]]=i;
        Update(1,1,n,i,1);
        cout<<Query(1,1,n,i,i)<<endl;
        while(idx<=q && query[idx].first==i)
        {
            int r=query[idx].first;
            int l=query[idx].second.first;
            int pos=query[idx].second.second;
            cout<<l<<" "<<r<<endl;
            ans[pos]=Query(1,1,n,l,r);
            idx++;
        }
    }
    for0(i,q)
