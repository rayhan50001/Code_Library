...
//}
int a[1000009];
void solve(int size,int kk)
{
    int max_so_far = INT_MIN, max_ending_here = 0,x,
       start =inf, end = 0, s=1;
    for (int i=2; i<=size; i++ )
    {
        ...
        {
            if (max_so_far < max_ending_here || (max_so_far == max_ending_here && (i - s > end - start)))
            {
                    start = s;
                    end = i;
            }
    ...
    if(max_so_far<0)printf("Route %d has no nice parts\n",kk);
