...
int main()
{
    ...
    {
      ll res=0;
      for(int j=0; j<=20; j++)//here it will count how much bit in each position,it will multiply by 2^(position), why because count represent how much bit are there. try to figure out with bitwise and operation. :p
      {
         if(!CHECK_BIT(arr[i],j))val[j]=0;
         else val[j]++;
         ///cout<<val[j]<<" ";
         res+=((ll)val[j]*(1<<(ll)j));
      }
