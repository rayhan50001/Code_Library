...
void make_hash(string s)
{
    ...
    {
        ll id=0;
        if(s[i-1]>='a' && s[i-1]<='z') id = s[i-1] - 'a';
        else id = s[i-1] - 'A' + 26;
        HASH[i]=(HASH[i-1]+(power[i-1]*(id+1))%mod)%mod;
    }
}
void poww()
{
    power[0]=1;
    for(int i=1; i<=1001; i++)power[i]=(power[i-1]*base)%mod;
}
int main()
{
    clock_t begin = clock();
    ...
    while(tt--)
    {
        ...
        for(int i=1; i<=len; i++)
        {
            ...
            {
                ll val=(HASH[j+i]+mod-HASH[j])%mod;
                //if(val<0)val+=mod;
                val=(val*power[len-j-1])%mod;
                if(val<0)val+=mod;
                s.insert(val);
            }
            ans+=s.size();
