...
int main()
{
    ...
    while(tt--)
    {
        ...
        sort(v.begin(),v.end());
        sort(findd.begin(),findd.end());
        for(int i=0; i<v.size(); i++)
        {
            // fuck i, i thought it's already sorted fuck
            int mx=0;
            if(i!=n-1)mx=max(mx,(v[i].fi+v[n-1].fi)%mod);
            else mx=max(mx,(v[i].fi+v[n-2].fi)%mod);
            int idx=upper_bound(v.begin(),v.end(),mp(mod-1-v[i].fi,n))-v.begin();
            idx--;
            //cout<<mx<<" "<<idx<<" "<<i<<" "<<v[i].fi<<" ";
            if(idx==i && idx-1>=0)
            {
                //cout<<"yes\n";
                mx=max(mx,(v[i].fi+v[idx-1].fi)%mod);
            }
            else if(idx>=0 && idx!=i)//fuck this condition
            {
               mx=max(mx,(v[i].fi+v[idx].fi)%mod);
            }
            //cout<<mx<<endl;
            ans[v[i].se]=mx;
        }
        for0(i,n){pf(ans[i]);printf(" ");}
        nl;
    }
   //end here
    clock_t end = clock();
