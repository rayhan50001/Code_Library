//sieve + (binary search - not needed) + implementation
#include<iostream>
#include<cstdio>
#include<list>
#include<string>
...
int main()
{
    ...
    while(sf2(l,r))
    {
        if(l==0 && r==0)break;
        if(r<l)swap(l,r);
        int st=lower_bound(prime.begin(),prime.end(),l)-prime.begin();
        int ed=upper_bound(prime.begin(),prime.end(),r)-prime.begin();
        //cout<<st<<" "<<ed<<endl;
        for(int i=st; i<ed; i++)
        {
            if(i+2<ed)
            {
                if(prime[i+1]-prime[i]==prime[i+2]-prime[i+1])
                {
                    ...
                    int suru=i;
                    int sesh=i+2;
                    while(sesh+1<prime.size() && prime[sesh+1]-prime[sesh]==diff)sesh++,i++;
                    if(prime[sesh]<=r)
                        {
                            if ( suru == 0 || prime [suru] - prime [suru - 1] != diff)
                            {
                            printf("%d",prime[suru]);
                            //cout<<suru<<" "<<sesh<<endl;
                            for(int j=suru+1; j<=sesh; j++)
                                printf(" %d",prime[j]);
                                nl;
                            }
                        }
