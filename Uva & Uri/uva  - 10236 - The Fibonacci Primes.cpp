//sieve + fibo => if index of prime n is also fibonacci prime. like prime[3]=5 and fib[5]=5 so it's prime.
#include<iostream>
#include<cstdio>
#include<list>
#include<string>
...
{
    pr[0]=pr[1]=1;
    for(int i=2; i*i<=2500001; i++)
    {
        if(pr[i]==0)
        {
            for(int j=i*i; j<=2500001; j+=i)
            {
                pr[j]=1;
            }
        }
