//hint/solution: http://codeforces.com/blog/entry/56261
#include<iostream>
#include<cstdio>
#include<list>
#include<string>
...
void pre()
{
    for(int g=1; g<=30000000; g++)
    {
        for(int x=1; g*x+g<=30000000; x++)
        {
            if((g*(x+1)^(g*x))==g)dp[g*(x+1)]++;
        }
    }
    for(int i=1; i<=30000000; i++)dp[i]+=dp[i-1];
}
int main()
{
    clock_t begin = clock();
