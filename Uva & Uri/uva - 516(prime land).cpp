...
template<typename T>inline vector<int> parse(T str)
{
    vector<int> res;
    int s;
...
int main()
{
    ...
    while(getline(cin,s))
    {
        ...
        {
            //cout<<powl(v[i],v[i+1])<<endl;
            sum=sum*powl(v[i],v[i+1]);
        }
        //cout<<sum<<endl;
        int num=sum-1;
        if(Factor::miller(num))
        {
            cout<<num<<" "<<1<<endl;
        }
        else
        {
           vector<pair<ll,ll> >fact = Factor::factorG(num);
           int n=fact.size();
           for(int i=n-1; i>=0; i--)
           {
